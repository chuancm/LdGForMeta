...
class Lid(BoundaryObject):
    """ Planar boundary with nematic on *both* sides. """
    def __init__(self, anch_cond, normal, height,radius,Cx):
        if normal == "x":
            normal = [1,0,0]
        elif normal == "y":
            normal = [0,1,0]
    ...
    def wall_member_func(self, X, Y, Z):
        Nx, Ny, Nz = self.normal
        position_dot_normal = X*Nx + Y*Ny + Z*Nz 
        return (position_dot_normal == self.height) & ((X-self.Cx)**2+(Y-self.Cy)**2 <= self.radius**2)
...
class CylindricalCapillary(BoundaryObject):
    def __init__(self, anch_cond, radius, period,height):
        self.radius = radius
        self.period = period
        self.Centerx=(period-1)/2
        self.Centery=self.Centerx
        self.base_center = np.array([(self.period-1)/2,(self.period-1)/2, 0])
        self.height=height
    ...
    def capillary_member_func(self, X, Y, Z):
        ...
        return ((X-self.Centerx)**2 + (Y-self.Centery)**2 <= self.radius**2) & (Z <=self.height)
